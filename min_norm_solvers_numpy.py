"""
This code is from Multi-Task Learning as Multi-Objective Optimization (Ozan Sener, Vladlen Koltun) - https://github.com/intel-isl/MultiObjectiveOptimization
"""
...
class MinNormSolverNumpy:
    ...
    def _min_norm_2d(vecs, dps):
        ...
        for i in range(len(vecs)):
            for j in range(i+1,len(vecs)):
                ...
                c,d = MinNormSolverNumpy._min_norm_element_from2(dps[(i,i)], dps[(i,j)], dps[(j,j)])
                if d < dmin:
                    dmin = d
                    sol = [(i,j),c,d]
        return sol, dps
    ...
    def find_min_norm_element(vecs):
        ...
        while iter_count < MinNormSolverNumpy.MAX_ITER:
            ...
            nc, nd = MinNormSolverNumpy._min_norm_element_from2(v1v1, v1v2, v2v2)
            new_sol_vec = nc*sol_vec + (1-nc)*new_point
            change = new_sol_vec - sol_vec
            # 确保 change 为 numpy 数组，并使用当前类定义的 STOP_CRIT
            if np.sum(np.abs(np.array(change))) < MinNormSolverNumpy.STOP_CRIT:
                return sol_vec, nd
            sol_vec = new_sol_vec
        return sol_vec, nd
